...
def search_notes(query):
    """Search notes by title: SAFE version using parameterized query."""
    conn = sqlite3.connect(DB_FILE)
    sql = "SELECT id, title, body FROM notes WHERE title LIKE ?"
    pattern = f"%{query}%"
    print(f"[DEBUG] Executing SQL: {sql} | params=({pattern!r},)")
    try:
...
def export_note(note_id):
    """Export a note to a file: SAFE version without shell."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.execute(
